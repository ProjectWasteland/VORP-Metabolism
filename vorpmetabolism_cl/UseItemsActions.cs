...
{
    internal class UseItemsActions : BaseScript
    {
        ...
        private void ConsumeItems(int index, string label)
        {
            ...
            {
                var newThirst = vorpmetabolism_init.pStatus["Thirst"].ToObject<int>() +
                                GetConfig.Config["ItemsToUse"][index]["Thirst"].ToObject<int>();
            ...
            {
                var newHunger = vorpmetabolism_init.pStatus["Hunger"].ToObject<int>() +
                                GetConfig.Config["ItemsToUse"][index]["Hunger"].ToObject<int>();
            ...
            {
                var newMetabolism = vorpmetabolism_init.pStatus["Metabolism"].ToObject<int>() +
                                    GetConfig.Config["ItemsToUse"][index]["Metabolism"].ToObject<int>();
            ...
            {
                var stamina = Function.Call<int>((Hash)0x36731AC041289BB1, API.PlayerPedId(), 1);
                var newStamina = stamina + GetConfig.Config["ItemsToUse"][index]["Stamina"].ToObject<int>();
            ...
            {
                var health = Function.Call<int>((Hash)0x36731AC041289BB1, API.PlayerPedId(), 0);
                var newhealth = health + GetConfig.Config["ItemsToUse"][index]["InnerCoreHealth"].ToObject<int>();
            ...
            {
                var health = Function.Call<int>((Hash)0x82368787EA73C0F7, API.PlayerPedId(), 0);
                var newhealth = health + GetConfig.Config["ItemsToUse"][index]["OuterCoreHealth"].ToObject<int>();
            ...
            {
                Function.Call((Hash)0xF6A7C08DF2E28B28, API.PlayerPedId(), 0,
                              GetConfig.Config["ItemsToUse"][index]["OuterCoreHealthGold"].ToObject<float>(), true);
            }

            if (GetConfig.Config["ItemsToUse"][index]["InnerCoreHealthGold"].ToObject<float>() != 0f)
            {
                Function.Call((Hash)0xF6A7C08DF2E28B28, API.PlayerPedId(), 0,
                              GetConfig.Config["ItemsToUse"][index]["InnerCoreHealthGold"].ToObject<float>(), true);
            }

            if (GetConfig.Config["ItemsToUse"][index]["OuterCoreStaminaGold"].ToObject<float>() != 0f)
            {
                Function.Call((Hash)0xF6A7C08DF2E28B28, API.PlayerPedId(), 1,
                              GetConfig.Config["ItemsToUse"][index]["OuterCoreStaminaGold"].ToObject<float>(), true);
            }

            if (GetConfig.Config["ItemsToUse"][index]["InnerCoreStaminaGold"].ToObject<float>() != 0f)
            {
                Function.Call((Hash)0xF6A7C08DF2E28B28, API.PlayerPedId(), 1,
                              GetConfig.Config["ItemsToUse"][index]["InnerCoreStaminaGold"].ToObject<float>(), true);
            }
        ...
        public async Task PlayAnimDrink(string propName)
        {
            ...
            var dict = "amb_rest_drunk@world_human_drinking@male_a@idle_a";
            var anim = "idle_a";
            ...
            var hashItem = (uint)API.GetHashKey(propName);

            var prop = API.CreateObject(hashItem, playerCoords.X, playerCoords.Y, playerCoords.Z + 0.2f, true, true,
                                        false, false, true);
            var boneIndex = API.GetEntityBoneIndexByName(API.PlayerPedId(), "SKEL_R_Finger12");

            await Delay(1000);

            Function.Call(Hash.TASK_PLAY_ANIM, API.PlayerPedId(), dict, anim, 1.0f, 8.0f, 5000, 31, 0.0f, false, false,
                          false);
            Function.Call(Hash.ATTACH_ENTITY_TO_ENTITY, prop, API.PlayerPedId(), boneIndex, 0.02f, 0.028f, 0.001f,
                          15.0f, 175.0f, 0.0f, true, true, false, true, 1, true);
            await Delay(6000);
        ...
        {
            var playerCoords = API.GetEntityCoords(API.PlayerPedId(), true, true);
            var dict = "mech_inventory@clothing@bandana";
            var anim = "NECK_2_FACE_RH";
